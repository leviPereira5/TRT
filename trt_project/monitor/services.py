...
# Ações de exemplo para a página inicial
...
def fetch_market_overview() -> dict:
    """Obtém dados de mercado para a página inicial."""
    result = {'us': [], 'br': [], 'fii': []}

    for symbol in FEATURED_US:
        try:
            t    = yf.Ticker(symbol)
            info = t.info
    ...
    for symbol in FEATURED_BR + FEATURED_FII:
        try:
            t    = yf.Ticker(symbol)
            info = t.info
            price = info.get("regularMarketPrice") or info.get("currentPrice") or info.get("previousClose")
            ...
            if symbol in FEATURED_FII:
                result['fii'].append(entry)
            else:
                result['br'].append(entry)
        except Exception as e:
            logger.warning(f"Erro ao obter {symbol}: {e}")

    return result

def get_last_price(stock: Stock) -> Decimal | None:
    last = stock.prices.first()
    return last.price if last else None

def calculate_variation(old_price: Decimal, new_price: Decimal) -> Decimal:
    if old_price == 0:
        return Decimal("0.00")
    return ((new_price - old_price) / old_price * 100).quantize(Decimal("0.01"))

def already_alerted(stock: Stock, direction: str) -> bool:
...
def send_alert_email(alert: Alert, settings: UserSettings):
    """Envia email de alerta (REQ-7)."""
    ...
    try:
        ...
        alert.save()
        logger.info(f"Email enviado para {settings.alert_email}: {alert.stock.symbol} {direction}")
    except Exception as e:
...
def monitor_stock(stock, settings):
    ...
    return {'symbol': stock.symbol, 'price': new_price,
            'old_price': old_price, 'variation': variation, 'alert': alert}
